...
async function startBot() {
  ...
  sock.ev.on('connection.update', (update) => {
    if (update.qr) {
      fs.writeFileSync(
        path.join(sessionDir, 'qr.json'),
        JSON.stringify({ qr: update.qr, ts: Date.now() })
      );
    ...
    if (update.code) {
      fs.writeFileSync(
        ...
        JSON.stringify({ code: update.code, ts: Date.now() })
      );
      console.log('Pairing code generated:', update.code);
    }
